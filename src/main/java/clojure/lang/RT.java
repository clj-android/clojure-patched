			...
			if("Dalvik".equals(System.getProperty("java.vm.name"))) {
				try {
					Class<?> loaderClass = Class.forName(
						"clojure.lang.AndroidDynamicClassLoader");
					Constructor<?> ctor = loaderClass.getConstructor(ClassLoader.class);
					return ctor.newInstance(baseLoader());
				} catch(ClassNotFoundException e) {
					// AndroidDynamicClassLoader not available (release build without
					// runtime-repl). Fall through to stock DynamicClassLoader, which
					// works fine for AOT-only builds where defineClass is never called.
				} catch(Exception e) {
					throw new RuntimeException(
						"Failed to create Android dynamic classloader", e);
				}
			}
